...
{
    public abstract class myprojWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static myprojWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(myprojWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<myprojDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<myprojDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<myprojDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<myprojDbContext, Task<T>> func)
        {
            T result;
